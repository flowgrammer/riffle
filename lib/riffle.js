...
var riffle = /* collection of stream utilities and hooks for a stream system */ function () {
  ...
  stream.join = function (/* optional */ func, /* optional */ config) {
    ...
    return stream(function (out) {
      ...
      if (config.canInvoke(saved, items)) {
        var params = [out].concat(arrayify(config.invokeWith(saved, items)));
        applyArgsToFn(func, params);
      };
    });
  };
  ...
  var funcBuilder = function (config) {
    return function (fn) {
      var joiner = stream.join(function (out) {
        var args = [].slice.call(arguments, 1);
        var result = fn.apply(global, args);
        out(result);
      }, config);
      /*
       Once the return value has been built and the join stream constructed,
       we use it for every invocation, unioning the arguments for ports,
       and using the same stream as a return value.
       */
      return function () {
        var args = [].slice.call(arguments);
        var invocation = new Array(args.length);
        var isInvoke = false;
        _.each(args, function (input, i) {
          if (stream.isStream(input)) {
            joiner.input(input, i);
          } else {
            isInvoke = true;
            invocation[i] = input;
          }
        });
        if (isInvoke) joiner.invoke.apply(null, invocation);
        return joiner;
      };
    };
  };
